...
def solve(funcs, a, b, rel_approx_tol=1.e-15, abs_approx_tol=1.e-12,
    ...
    rel_approx_tol : float or list
        The relative tolerance used in the approximation tolerance. The
        error is bouned by
    ...
    a = np.float64(a)
    b = np.float64(b)
    # Choose an appropriate max degree for the given dimension if none
    # is specified.
    if deg is None:
        deg_dim = {1: 100, 2: 20, 3: 20, 4: 16}
        if dim > 4:
            deg = 10
        else:
            deg = deg_dim[dim]
...
def transform(x, a, b):
    ...
    a : float or numpy array
        The lower bound on the interval. Float if one-dimensional, numpy
        array if multi-dimensional.
    b : float or numpy array
        The upper bound on the interval. Float if one-dimensional, numpy
        array if multi-dimensional.
...
def initialize_values_arr(dim, deg):
    ...
    Returns
    -------
    An empty numpy array that can be used to hold values for a
    chebyshev_block_copy of dimension dim degree < deg.
    """
...
def values_arr(dim):
    ...
    -------
    An empty numpy array that can be used to hold values for a
    chebyshev_block_copy of dimension dim and degree less than the
    degree used for initialize_values_arr.
    """
...
def block_copy_slicers(dim, deg):
    ...
    cheb_slicers : list of tuples of slice objects
        Slice objects used to index into the array we're copying
        evaluations to.
    slicer : tuple of slice objets
        Used to index into the portion of that array we're using for the
        fft input.
    """
    block_slicers = []
    cheb_slicers = []
...
def interval_approximate_1d(f, a, b, deg, return_bools=False, return_inf_norm=False):
    ...
    if return_bools:
        ...
        if return_inf_norm:
            return coeffs[:deg+1], sign_change, inf_norm
        else:
            return coeffs[:deg+1], sign_change
    else:
        if return_inf_norm:
...
def interval_approx_slicers(dim, deg):
    """Helper function for interval_approximate_nd. Builds slice objects
    to index into the output of the fft and divide some of the values by
    2 and turn them into coefficients of the approximation.
    ...
    slices : tuple of slice objets
        Used to index into the portion of the array that are
        coefficients
    rescale : int
        amount to rescale the evaluations by in order to feed them into
        the fft
    """
...
def get_subintervals(a, b, dimensions, interval_data, polys, approx_error, check_subintervals=False):
    ...
    check_subintervals : bool
        If True runs the subinterval checks to throw out intervals where
        the functions are never 0.

    Returns
    -------
    subintervals : list
        Each element of the list is a tuple containing an a and b, the
...
def good_zeros_nd(zeros, imag_tol, real_tol):
    ...
    imag_tol : float
        How large the imaginary part can be to still have it be
        considered real.
    real_tol : float
        How far the real part can be outside the interval [-1, 1]^n and
...
def get_abs_approx_tol(func, deg, a, b, dim):
    """ Gets an absolute approximation tolerance based on the assumption that
        ...
        ----------
            ...
            deg : int
                The degree to use to approximate the function on the
\                interval.
            a : numpy array
                The lower bounds of the interval on which to
\                approximate.
            b : numpy array
                The upper bounds of the interval on which to
\                approximate.

        Returns
        -------
            abs_approx_tol : float
                The calculated absolute approximation tolerance based
                on the noise of the function on the small interval.
    """
...
def deg_slices(deg, dim):
    """Helper function for get_abs_approx_tol. Returns a slice object
    for accessing all the terms of total degree less than deg in a
    coefficient tensor.
    ...
    -------
        slice
            The slice that accesses all the coefficients of degree less
            than deg.
    """
...
def random_point(dim):
    """Gets a random point from [-1, 1]^dim that's used for
    get_abs_approx_tol.
...
def subdivision_solve_nd(funcs, a, b, deg, interval_data,
                         root_tracker, tols, max_level, good_degs=None, 
                         level=0, use_target_tol=False,
                         trust_small_evals=False):
    ...
    use_target_tol : bool
        ...
        build the linear system used to find the roots.
    """
    if level >= max_level:
    ...
    for func, good_deg in zip(funcs, good_degs):
        ...
        if coeff is None:
            ...
            for new_a, new_b in intervals:
                subdivision_solve_nd(funcs, new_a, new_b, deg, interval_data, root_tracker, tols, max_level, level=level+1, trust_small_evals=trust_small_evals)
            return
        else:
    ...
    #  If trim_coeffs introduced too much error, subdivide
    if not good_approx:
        intervals = get_subintervals(a, b, get_div_dirs(dim), interval_data, cheb_approx_list, approx_errors, True)
        for new_a, new_b in intervals:
            subdivision_solve_nd(funcs, new_a, new_b, deg, interval_data, root_tracker, tols, max_level, good_degs, level+1, trust_small_evals=trust_small_evals, use_target_tol=True)

    # If any of the approximations are not linear, subdivide again
    elif np.any(np.array([coeff.shape[0] for coeff in coeffs]) > 2):
        intervals = get_subintervals(a, b, get_div_dirs(dim), interval_data, cheb_approx_list, approx_errors, True)
        for new_a, new_b in intervals:
            subdivision_solve_nd(funcs, new_a, new_b, deg, interval_data, root_tracker, tols, max_level, good_degs, level+1, trust_small_evals=trust_small_evals, use_target_tol=True)

    # Check if any approx error is greater than target_tol for linear system
    ...
    # Check if everything is linear
        ...
        if deg != 2:
            subdivision_solve_nd(funcs, a, b, 2, interval_data, root_tracker, tols, max_level, good_degs, level, trust_small_evals=trust_small_evals, use_target_tol=True)
            return
...
def subdivision_solve_1d(f, a, b, deg, interval_data, root_tracker,
                         tols, max_level, level=0,
                         trust_small_evals=False):
