...
from yroots.MacaulayReduce import reduce_macaulay_qrt, find_degree, add_polys,\
                                  reduce_macaulay_tvb, reduce_macaulay_svd
from yroots.utils import row_swap_matrix, slice_top, get_var_list, mon_combos,\
                         mon_combosHighest, condeigs, solve_linear, memoize
from scipy.stats import ortho_group


def multiplication(polys, max_cond_num, verbose=False, return_all_roots=True,
                   method='svd'):
    """Finds the roots of the given list of multidimensional polynomials
    using a multiplication matrix.
    ...
    return_all_roots : bool
        If True returns all the roots, otherwise just the ones in the
        unit box.
    ...
    if len(polys) == 1:
        ...
        return solve(polys[0], MSmatrix=0)
    poly_type = is_power(polys, return_string=True)
    dim = polys[0].dim

    # By Bezout's Theorem. Useful for making sure that the reduced
    # Macaulay Matrix is as we expect
    ...
    if cut == 0:
        ...
        # M is a 3d array containing the multiplication-by-x_i matrix
        # in M[..., i]
        ...
        # Compute the roots using eigenvalues of the Möller-Stetter
        # matrices
...
def indexarray(matrix_terms, m, var):
    ...
    matrix_terms : 2d integer ndarray
        Array containing the monomials in order. matrix_terms[i] is the
        array containing the exponent for each variable in the ith
        multivariate monomial.
    m : int
        Number of monomials of highest degree, i.e. those that do not
    ...
    arr : 1d integer ndarray
        Array containing the indices of the lower-degree monomials after
        multiplication by x_var
    """
...
def indexarray_cheb(matrix_terms, m, var):
    """Compute the array mapping Chebyshev monomials under
    multiplication by x_var:
    ...
    matrix_terms : 2d integer ndarray
        Array containing the monomials in order. matrix_terms[i] is the
        array containing the degree for each univariate Chebyshev
        monomial in the ith multivariate monomial
    m : int
        Number of monomials of highest degree, i.e. those that do not
        need to be multiplied.
    var : int
...
def ms_matrices(E, Q, matrix_terms, dim):
    ...
    ----------
    E : (m, k) ndarray
        Columns of the reduced Macaulay matrix corresponding to the
        quotient basis.
    Q : (l, n) 2d ndarray
        Matrix whose columns give the quotient basis in terms of the
        monomial basis.
    ...
    -------
    M : (n, n, dim) ndarray
        Array containing the nxn Möller-Stetter matrices, where the
        matrix corresponding to multiplication by x_i is M[..., i].
    """
    n = Q.shape[1]
    m = E.shape[0]
    M = np.empty((n, n, dim))
...
def ms_matrices_p_cheb(E, P, matrix_terms, dim, cut):
    ...
    E : (m, k) ndarray
        ...
        quotient basis.
    P : (, l) ndarray
        Array of pivots returned in QR with pivoting, used to permute
        the columns.
    matrix_terms : 2d ndarray
        Array with ordered Chebyshev basis.
    dim : int
...
def sort_eigs(eigs, diag):
    ...
    diag : 1d complex ndarray
        Array containing the diagonal of the approximate Schur
        factorization.
...
@memoize
def get_Q_c(dim):
    """Generates a once-chosen random orthogonal matrix and a random
    linear combination for use in the simultaneous eigenvalue compution.
...
def msroots(M):
    """Computes the roots to a system via the eigenvalues of the
    Möller-Stetter matrices. Implicitly performs a random rotation of
    the coordinate system to avoid repeated eigenvalues arising from
    special structure in the underlying polynomial system. Approximates
    the joint eigenvalue problem using a Schur factorization of a linear
    ...
    -------
    roots : (n, dim) ndarray
        Array containing the approximate roots of the system, where each
        row is a root.
    """
    dim = M.shape[-1]
    ...
    # Compute the matrix U that triangularizes a random linear
    # combination
    U = schur((M*c).sum(axis=-1), output='complex')[1]

    # Compute the eigenvalues of each matrix, and use the computed U
    #  o sort them
    T = (U.conj().T)@(M[..., 0])@U
    w, v = eig(M[..., 0])
    ...
    # (will be the same for all matrices)
...
def build_macaulay(initial_poly_list, verbose=False):
    """Constructs the unreduced Macaulay matrix. Removes linear
    polynomials by substituting in for a number of variables equal to
    the number of linear polynomials.
    ...
    matrix : 2d ndarray
        The Macaulay matrix.
    matrix_terms : 2d integer ndarray
        Array containing the ordered basis, where the ith row contains
        the exponent/degree of the ith basis monomial.
    cut : int
        Where to cut the Macaulay matrix for the highest-degree
        monomials.
    varsToRemove : list
        The variables removed with removing linear polynomials.
    A : 2d ndarray
        A matrix giving the linear relations between the removed
        variables and the remaining variables.
    Pc : 1d integer ndarray
        Array containing the order of the variables as the appear in the
        columns of A.
    """
...
def makeBasisDict(matrix, matrix_terms, VB, power):
    ...
    This is a dictionary of the terms on the diagonal of the reduced
    Macaulay matrix to the terms in the Vector Basis.
    ...
    matrix_terms : numpy array
        The terms in the matrix. The i'th row is the term represented by
        the i'th column of the matrix.
    VB : numpy array
        Each row is a term in the vector basis.
    power : bool
        If True, the initial polynomials were MultiPower. If False, they
        were MultiCheb.

    Returns
    -----------
    basisDict : dict
        Maps terms on the diagonal of the reduced Macaulay matrix
        (tuples) to numpy arrays that represent the terms reduction into
        the Vector Basis.
    """
    basisDict = {}
    ...
    # We don't actually need most of the rows, so we only get
    # the ones we need
    if power:
        neededSpots = set()
        for term, mon in product(VB, get_var_list(VB.shape[1])):
...
def create_matrix(poly_coeffs, degree, dim):
    ...
    ----------
    poly_coeffs : list.
        Contains numpy arrays that hold the coefficients of the
        polynomials to be put in the matrix.
    degree : int
        The degree of the Macaulay Matrix.
    dim : int
        The dimension of the polynomials going into the matrix.
    varsToRemove : list
        The variables to remove from the basis because we have
        linear polysnomials.
    Returns
    -------
    matrix : 2D numpy array
        The Macaulay matrix.
    matrix_terms : numpy array
        The ith row is the term represented by the ith column of
        the matrix.
    cut : int
    ...
    matrix_terms, cut = sorted_matrix_terms(degree, dim)

    # Get the slices needed to pull the matrix_terms from
    # the coeff matrix.
    matrix_term_indexes = list()
    for row in matrix_terms.T:
        matrix_term_indexes.append(row)

    # Adds the poly_coeffs to flat_polys, using added_zeros to
    # make sure every term is in there.
...
def sorted_matrix_terms(degree, dim):
    """Finds the matrix_terms sorted in the term order needed for
    Macaulay reduction. So the highest terms come first, the x, y, z etc
    monomials last.
    ...
    sorted_matrix_terms : numpy array
        The sorted matrix_terms. The ith row is the term represented by
    ...
    # normal case
        ...
        matrix_terms = np.reshape(highest_mons+other_mons+xs_mons,
                                 (len(highest_mons+other_mons+xs_mons), dim))
...
def _random_poly(_type, dim):
    ...
    c_1x_1 + c_2x_2 + ... + c_nx_n where n = dim and each c_i is a
