...
from yroots.polynomial import is_power
from yroots.Multiplication import multiplication
from yroots.utils import match_poly_dimensions, ConditioningError


def solve(polys, MSmatrix=0, eigvals=True, verbose=False,
          return_all_roots=True, max_cond_num=1.e6, method='svd'):
    """
    ...
    MSmatrix : int
        ...
        For a univariate polynomial, the options are:
            0 (default)
                The companion or colleague matrix, rotated 180 degrees
            1
                The unrotated companion or colleague matrix
            -1
                The inverse of the companion or colleague matrix
        For a multivariate polynomial, the options are:
            0 (default)
                The Moller-Stetter matrix of a random polynomial
            Some positive integer i <= dimension
                The Moller-Stetter matrix of x_i, where variables are
                index from x1, ..., xn
            Some negative integer i >= -dimension
                The Moller-Stetter matrix of x_i-inverse
    eigvals : bool
        Whether to compute roots of polynomials from eigenvalues (True)
        or eigenvectors (False).
    verbose : bool
