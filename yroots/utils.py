# A collection of functions used in the Macaulay and TVB solvers
import numpy as np
import itertools
...
class TooManyRoots(Exception):
    """Raised when the number of roots found by the Macaulay matrix
    exceeds the Bezout bound.
...
def sorted_polys_monomial(polys):
    ...
    Parameters
    ----------
    polys : array-like, contains polynomial objects
    ...
    """
    # A list to contain the number of monomials with non zero
    # coefficients.
    num_monomials = []
    for poly in polys:
        # This gets the length of the list of first indexes, since
        # that is number of non-zero coefficients in the coefficient
        # array.
...
def row_linear_dependencies(matrix, accuracy=1.e-10):
    ...
    if numMissing == 0:  # Full Rank. All rows independent
        ...
        # Find the rows we can take out. These are ones that are
        # non-zero in the last rows of Q transpose, since Q.T*A=R.
...
def triangular_solve(matrix):
    """
    Takes a matrix that is in row echelon form and reduces it into row
    ...
    # List to keep track of original index of the columns in c.
    order_c = []
    # List to keep track of the original index of the columns in d.
    order_d = []
    ...
    if m != n:
        ...
        # C will be the square matrix that is upper triangular with no
...
def solve_linear(coeffs):
    ...
    try:
        ...
        if str(e) == 'Singular matrix':
            # if the system is dependent, then there are infinitely many
            # roots if the system is inconsistent, there are no roots.
...
def first_x(string):
    """Finds the first position of an 'x' in a string. If there is not x
    ...
    -------
    i : int
        The position in the string of the first 'x' character. If 'x'
        does not appear in the string the return value is the length of
        the string.
    """
...
def makePolyCoeffMatrix(inputString):
    """
    Takes a string input of a polynomaial and returns the coefficient
    matrix for it. Usefull for making things of high degree of dimension
    so you don't have to make it by hand.

    All strings must be of the following syntax.
    Ex. '3x0^2+2.1x1^2*x2+-14.73x0*x2^3'

    1. There can be no spaces.
    2. All monomials must be seperated by a '+'. If the coefficient of
       the monomial is negative then the '-' sign should come after the
       '+'. This is not needed for the first monomial.
    3. All variables inside a monomial are seperated by a '*'.
    4. The power of a variable in a monomial is given folowing
       a '^' sign.
    """
    matrixSpots = list()
    coefficients = list()
    ...
    matrixSize = matrixSize + np.ones_like(matrixSize)
     # So the variables are in the right order.
    matrixSize = matrixSize[::-1]
    matrix = np.zeros(matrixSize)
    for i in range(len(matrixSpots)):
        # So the variables are in the right order.
        matrixSpot = matrixSpots[i][::-1]
...
def slice_top(matrix_shape):
    """Gets the n-d slices needed to slice a matrix into the top corner
    of another.
    ...
    -------
    slices : list
        Each value of the list is a slice of the matrix in some
        dimension. It is exactly the size of matrix_shape.
    """
    slices = list()
...
def slice_bottom(matrix):
    """ Gets the n-d slices needed to slice a matrix into the bottom
    corner of another.
    ...
    slices : list
        Each value of the list is a slice of the matrix in some
        dimension. It is exactly the size of the matrix.
    """
    slices = list()
...
def _fold_in_i_dir(solution_matrix, dim, fdim, size_in_fdim, fold_idx):
    ...
    # Matrix of zeroes used to insert the new values.
    sol = np.zeros_like(solution_matrix) 
    slice_0 = slice(None, 1, None)  # index to take first slice
    # index to take slice that contains the axis folding around.
    ...
    # makes first slice in sol equal to the slice we fold around 
    # in solution_matrix
    sol[indexer1] = solution_matrix[indexer2]

    # Loop adds the slices above and below the slice we rotate around 
    # and inserts solutions in sol.
    for n in range(size_in_fdim):
        # Used to input new values in sol.
        slice_2 = slice(n+1, n+2, None)
        # Used to find slices that are n above fold_idx
        slice_3 = slice(fold_idx+n+1, fold_idx+n+2, None)
        # Used to find slices that are n below fold_idx
        ...
        # if statement checks to ensure that slices to be added are
...
def _mon_mult1(initial_matrix, idx, dim_mult):
    ...
    idx : tuple of ints
        The index of a monomial of one variable to multiply by
        initial_matrix.
    dim_mult : int
    ...
    -------
    ndarray
        Coeff that are the result of the one dimensial monomial
        multiplication.
...
def mon_combos(mon, numLeft, spot=0):
    """Finds all the monomials up to a given degree and returns them.
    Works recursively.

    Parameters
    --------
    mon: list
        A list of zeros, the length of which is the dimension of the
        desired monomials. Will change as the function searches
        recursively.
    numLeft : int
        The degree of the monomials desired. Will decrease as the
        function searches recursively.
    spot : int
        The current position in the list the function is iterating
        through. Defaults to 0, but increases in each step of the 
        recursion.
...
def num_mons_full(deg, dim):
    """Returns the number of monomials of a certain dimension and less
    than or equal to a certian degree.
...
def sort_polys_by_degree(polys, ascending = True):
    ...
    ascending : bool
        Defaults to True. If True the polynomials are sorted in order of
        ascending degree. If False they are sorted in order of
        descending degree.
    Returns
    -------
    sorted_polys : list
...
def deg_d_polys(polys, deg, dim):
    ...
    Iterating through this for each needed degree creates a full rank
    matrix in all dimensions, getting rid of the extra rows that are
    there when we do all the monomial multiplications.

    The idea behind this algorithm comes from that cool triangle thing I
    drew on a board once, I have no proof of it, but it seems to work
    real well.
    ...
    poly_coeff_list : list
        A list of the polynomials of degree deg to be added to the
        Macaulay Matrix.
    """
    ignoreVar = 0
    poly_coeff_list = list()
...
def arrays(deg, dim, mon):
    ...
    dim: int
        The dimension of the polynomials in the Macaualy matrix that the
        row is in.
    mon: int
    ...
    -------
    arrays : numpy array
        The array is full of True/False values, using np.where the array
        is True will generate the permutation array.
    """
    if dim-1==mon:
...
def permutation_array(deg, dim, mon):
    """Finds the permutation array to multiply a row of a matrix by a
    certain monomial.
    ...
    permutation_array : numpy array
        Permutting a row in the Macaulay matrix by this array will be
        equivalent to multiplying by mon.
    """
    if mon == dim -1:
        array = [False]
        ...
        for d in range(2, deg+1):
            ...
            array = first+array
    return np.array(inverse_P(np.hstack((np.where(~np.array(array))[0],
                                         np.where(array)[0]))))


def all_permutations(deg, dim, matrixDegree, permutations=None,
                     current_degree=2):
    """Finds all the permutation arrays needed to create a Macaulay
    Matrix.

    Parameters
    ----------
    deg: int
        Permutation arrays will be computed for all monomials up to this
        degree.
    dim: int
        The dimension the monomials for which permutation degrees.
    matrixDegree: int
        The degree of the Macaulay Matrix that will be created. This is
        needed to get the length of the rows.
    permutations: dict
        Defaults to none. The permutations that have already been
        computed.
    current_degree: int
        Defaults to 2. The degree of permutations that have already been
        computed.
    Returns
    -------
    permutations : dict
        The keys of the dictionary are tuple representation of the
        monomials, and each value is the permutation array corresponding
        to multiplying by that monomial.
    """
...
def cheb_perturbation3(mult_mon, mons, mon_dict, var):
    """
    Calculates the Cheb perturbation for the case where mon is greater
    than poly_mon.
...
def newton_polish(polys, root, niter=100, tol=1e-5):
    ...
    ----------
    polys : list
        A list of polynomial objects of the same type
        (MultiPower or MultiCheb).
    root : ndarray
        An initial guess for Newton's method, intended to be a candidate
        root from root_finder.
    niter : int
    ...
    -------
    x1 : ndarray
        The terminal point of Newton's method, an estimation for a root
        of the system.
    """
    m = len(polys)
    dim = max(poly.dim for poly in polys)
...
class Tolerances:
    ...
    A tolerance may be a float or an iterable type 
    (ex. list, numpy array, etc). If an iterable is used, all iterables 
    passed in must be the same length. Any floats passed in will be
    resized into lists of the same length as the iterables being used.

    If a tolerance of name "tol" is passed in, the object self.tols is 
    created to store all or the values to use for "tol". self.tol will 
    be the current value for "tol".

    ---DEVELOPER WARNING---
    IF ANY OTHER ATRRIBUTE IS CREATED OF ITERABLE TYPE THIS CLASS MAY 
    CRASH!!!
...
def condeigs(A, w, v, condvec=False):
    """Estimates the condition numbers of the eigenvalues of A.
    Optionally estimates the condition numbers of the eigenvectors."""
    n = A.shape[0]
...
def householder(x):
    """Given a vector x, computes a Householder reflector Q such that
    the first column of (Q^H)AQ is a multiple of e_1, whenever x is an
    eigenvector of A.
    """
