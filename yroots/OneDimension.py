...
def solve(poly, MSmatrix=0, eigvals=True, verbose=False):
    ...
    if MSmatrix not in [-1, 0]:
        raise ValueError(('MSmatrix must be -1 (inverse companion), or 0 '
                          '(rotated companion)'))
    ...
    if type(poly) == MultiPower:
        ...
        if MSmatrix == 0:
            # MultiPower is rotated 180 so it plays nice with
            # hessenberg properties
...
def multPower(coeff, eigvals=True, verbose=False):
    """Finds the zeros of a 1-D power polynomial using a rotated
...
def divPower(coeff, eigvals=True, verbose=False):
    """Finds the zeros of a 1-D power polynomial using a division
    matrix.
...
def multCheb(coeff, eigvals=True, verbose=False):
    """Finds the zeros of a 1-D chebyshev polynomial using a
    multiplication matrix.
...
def divCheb(coeff, eigvals=True, verbose=False):
    """Finds the zeros of a 1-D chebyshev polynomial using a division
    matrix.
